...
class SteinIdentity(nn.Module):
    def __init__(self, sigma=1e-4, fd_format="forward", args=None):
        ...
        with torch.no_grad():
            ...
            if self.fd_format == "forward":
                for name in original_parameter.keys():
                    delta[name] = torch.zeros(original_parameter[name].size()).cuda(self.args.device)
                    grad[name] = torch.zeros(original_parameter[name].size()).cuda(self.args.device)
                for i in range(self.K):
                    for name in delta.keys():
                        delta[name] = self.sigma * torch.randn(delta[name].size()).cuda(self.args.device)
                        original_parameter[name] = torch.tensor(original_parameter[name], dtype=torch.float32)
