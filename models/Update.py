...
class LocalUpdate_SFL:
    def __init__(self, args, dataset=None, idxs=None):
        ...
        self.selected_clients = []
        self.ldr_train = DataLoader(DatasetSplit(dataset, idxs), batch_size=self.args.local_bs, shuffle=True)
        self.estimate_grad = args.estimate_grad

    def splitTrain(self, net_client, net_server):
        if not self.estimate_grad:  # edited 1130
            net_client.train()
            optimizer_client = torch.optim.SGD(net_client.parameters(), lr=self.args.lr, momentum=self.args.momentum,
                                               weight_decay=self.args.weight_decay)
        ...
        for epIdx in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                # edited 1130
                if self.estimate_grad:
                    all_fx = net_client(images, labels, True)
                else:
                    all_fx = net_client(images)

                net_client.zero_grad()

                fx_to_server = all_fx.clone().detach().requires_grad_(not self.estimate_grad)  # edited 1130
